...
-- This version uses TEXT instead of enum for plan_type
...
BEGIN
    ...
    SET remaining_letters = CASE
            WHEN plan = 'standard_4_month' THEN 4
            WHEN plan = 'premium_8_month' THEN 8
            ELSE remaining_letters -- one_time doesn't reset
        END,
        last_reset_at = NOW(),
        updated_at = NOW()
    WHERE status = 'active'
      AND plan IN ('standard_4_month', 'premium_8_month')
      AND DATE_TRUNC('month', last_reset_at) < DATE_TRUNC('month', NOW());
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to add allowances on purchase (using TEXT instead of enum)
